...
function challengeCountForTierProgress(id) {
    const user = db[id];
    let i = 0;
    Object.keys(user.currentChallengeProgress).forEach(challenge => {
        if (user.currentChallengeProgress[challenge] > 1) i++;
    });
    return i;
}
...
* **Please click the button below to create your account and type your name inside the modal that will pop up upon pressing!**
...
**Account Created:** :calendar: <t:${Math.floor(user.created/1000)}:R>
## **Badges:**
${badgeDescriptions}
                        `)
                                    .addFields(
...
* ${icons.xp} **You are currently Level \`${level}\`**
...
`)
        ...
        if (message.content.startsWith('?unmod')) {
            try {
                ...
                user.badges.mod = 0;
                if (user.badges.verified == 1) user.accountType = 2;
                if (user.badges.verified < 1) user.accountType = 1;
        ...
        if (message.content.startsWith('?reqban')) {
            try {
                ...
                if (!user.canUseRequest) return message.reply({
                    embeds: [
                        new EmbedBuilder()
                            .setTitle(':no_entry_sign: This user is already request banned!')
                            .setColor('Red')
                    ]
                });
                user.canUseRequest = false;
                return message.reply({
                    embeds: [
                        new EmbedBuilder()
...
* **Example:** If you want set the entire map to wood blocks, you do \`?editor fill 0,0,19,8,WOOD,1000\`
                ...
                if (action == 'export') {
                    ...
                    const mapName = `${editors[message.author.id].title}-${id}`.replaceAll(' ', '-').replaceAll('/', '-').replaceAll('.', '-').replaceAll('\\', '-'); // '-' looks like a silly face woaa -w-
                    await WawaUtils.editjsonf(`./maps/${mapName}.json`, editors[message.author.id]);
                    return message.reply({
                        content: `
...
:exclamation: ***__Please make sure to save this somewhere where you can access it if anything happens to your Discord account!__***
    ...
    if (interaction.isModalSubmit() && interaction.customId.startsWith('registerModal_')) {
        try {
            ...
            if (names[ign.toLowerCase().replaceAll(' ', '-')]) return await interaction.reply({content: `:no_entry_sign: **This name is already taken! Please click the button again and re-submit the modal with a different name**`, flags: 64});
